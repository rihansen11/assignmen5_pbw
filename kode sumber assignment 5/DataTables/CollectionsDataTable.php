...
class CollectionsDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addColumn('action', function ($collection) {
            return '<a href="/koleksiView/' . $collection->id . '" class="btn btn-primary">View</a>';
        })
        ->setRowId('id');
    }
    ...
    public function query(Collection $model): QueryBuilder
    {
        return $model->newQuery()->select([
            ...
            'jumlahKoleksi',
            'createdAt',
            DB::raw('IFNULL(jumlahSisa, 0) as jumlahSisa'),
            DB::raw('IFNULL(jumlahKeluar, 0) as jumlahKeluar'),
            'updated_at'
        ]);
    }
